...
//         'dealer_name',
  ...
  return Transaction::select(
    ...
    public function getTransactionDetails($orderid){
        ...
        return Transaction::with(['employee:employee_slug,full_name,employee_no'])->join('branch_stocks', 'branch_stocks.item', '=', 'transactions.model_no')->where('order_id',$orderid)
        ->select('branch_stocks.description','order_id','transactions.created_at','promo_code','product_type','model_no','price_type','offer_type','offer_qty','order_qty','offer_price','promo_code','rm_name','dealer_code','dealer_name','product_type','model_no','order_price','order_id','status','stock','transactions.billed_qty','region','sales_slug','modified_by','ordered_by')
        ->get();
