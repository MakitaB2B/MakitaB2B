...
@section('container')
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                                    ...
                                    <div class="float-left">
                                        ...
                                        <select class="card-title" id="repparts" style="margin-left: 20px;">
                                            ...
                                            @if (count($replacedParts) > 0)
                                                @if ($replacedParts[0]->oldno != '')
                                                    <option value="{{ Crypt::encrypt($replacedParts[0]->oldno) }}">Old No:
                                                        {{ $replacedParts[0]->oldno }}</option>
                                                @endif
                                                @if ($replacedParts[0]->replace1 != '')
                                                    <option value="{{ Crypt::encrypt($replacedParts[0]->replace1) }}">
                                                        Replace-1: {{ $replacedParts[0]->replace1 }}</option>
                                                @endif
                                                @if ($replacedParts[0]->replace2 != '')
                                                    <option value="{{ Crypt::encrypt($replacedParts[0]->replace2) }}">
                                                        Replace-2: {{ $replacedParts[0]->replace2 }}</option>
                                                @endif
                                                @if ($replacedParts[0]->replaced3 != '')
                                                    <option value="{{ Crypt::encrypt($replacedParts[0]->replaced3) }}">
                                                        Replace-3: {{ $replacedParts[0]->replaced3 }}</option>
                                                @endif
                                            @endif
                                        </select>
                                    </div>
                                    <div class="float-right">
                                        <select class="card-title" id="stype" style="width: 129px;margin-right:10px">
                ...
                @else
                    <h1 class="card-title"> {{ $errorMsg }}</h1>
                @endif
    ...
    @push('scripts')
        ...
        <script>
            $('#repparts').on('change', function() {
                let replaced = $.trim($("#repparts").val());
                window.location.href = "{{ url('admin/branch-stock-details/') }}/" + replaced;
            });
                ...
                if (searchType == '') {
                    ...
                    if (searchtxt != '' && searchtxt.length > 3) {
                        ...
                        $.ajax({
                            ...
                            type: 'post',
                            data: 'searchtxt=' + searchtxt + '&searchtype=' + searchType + '&searchFrom=' +
                                searchFrom +
                                '&_token={{ csrf_token() }}',
